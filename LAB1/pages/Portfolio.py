...
def about_me_section():
    st.header("About Me")
    st.image(info.profile_picture, width = 100)
...
def side_bar():
    ...
    st.sidebar.text("Connect to me on Linkedin")
    ...
    email_link = f'<a href="{info.my_email_address}"><img src="{info.email_image_url}" alt = "Email" width = "75" height = "75"></a>'
...
def experience_section(experience_data):
    st.header("Professional Expereince")
...
def skill_section(programming_data,spoken_data):
    st.header("Skills")
    ...
    st.write("---")
skill_section(info.programming_data,info.spoken_data)
